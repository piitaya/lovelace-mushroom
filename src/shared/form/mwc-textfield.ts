...
export abstract class TextFieldBase extends FormElement {
    ...
    protected renderRipple(): TemplateResult | string {
        return this.outlined ? "" : html` <span class="mdc-text-field__ripple"></span> `;
    }
            ...
            : html`
                  <span
                      .floatingLabelFoundation=${floatingLabel(
                          this.label
                      ) as unknown as MDCFloatingLabelFoundation}
                      id="label"
                      >${this.label}</span
                  >
              `;
    ...
    protected renderIcon(icon: string, isTrailingIcon: boolean = false): TemplateResult {
        ...
        return html`<i class="material-icons mdc-text-field__icon ${classMap(classes)}"
            >${icon}</i
        >`;
    }
    ...
    /** @soyTemplate */
    protected renderAffix(content: string, isSuffix: boolean = false): TemplateResult | string {
        /** @classMap */
        const classes = {
        ...
        return html`<span class="mdc-text-field__affix ${classMap(classes)}"> ${content}</span>`;
    }
    ...
    protected renderLineRipple(): TemplateResult | string {
        return this.outlined
            ...
            : html`
                  <span
                      .lineRippleFoundation=${lineRipple() as unknown as MDCLineRippleFoundation}
                  ></span>
              `;
    }
    ...
    protected renderHelperText(
        ...
        const ariaHiddenOrUndef =
            this.focused || this.helperPersistent || showValidationMessage ? undefined : "true";
    ...
    protected renderCharCounter(shouldRenderCharCounter: boolean): TemplateResult | string {
        ...
        return !shouldRenderCharCounter
            ? ""
            : html` <span class="mdc-text-field-character-counter"
    ...
    protected getInputAdapterMethods(): MDCTextFieldInputAdapter {
        return {
            ...
            isFocused: () =>
                this.shadowRoot ? this.shadowRoot.activeElement === this.formElement : false,
            registerInputInteractionHandler: (evtType, handler) =>
                this.formElement.addEventListener(evtType, handler, {
    ...
    protected getLabelAdapterMethods(): MDCTextFieldLabelAdapter {
        return {
            floatLabel: (shouldFloat: boolean) =>
                this.labelElement && this.labelElement.floatingLabelFoundation.float(shouldFloat),
            getLabelWidth: () => {
                return this.labelElement ? this.labelElement.floatingLabelFoundation.getWidth() : 0;
            },
            hasLabel: () => Boolean(this.labelElement),
            shakeLabel: (shouldShake: boolean) =>
                this.labelElement && this.labelElement.floatingLabelFoundation.shake(shouldShake),
    ...
    override firstUpdated() {
        ...
        this.outlineElement?.updateComplete.then(() => {
            ...
            this.outlineWidth = this.labelElement?.floatingLabelFoundation.getWidth() || 0;
        });
    }
