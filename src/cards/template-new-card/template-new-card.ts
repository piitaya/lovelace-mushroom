...
import { TEMPLATE_CARD_NEW_EDITOR_NAME, TEMPLATE_CARD_NEW_NAME } from "./const";
import { TemplateNewCardConfig } from "./template-new-card-config";

registerCustomCard({
  type: TEMPLATE_CARD_NEW_NAME,
...
export class Template extends LitElement implements LovelaceCard {
  public static async getConfigElement(): Promise<LovelaceCardEditor> {
    await import("./template-new-card-editor");
    return document.createElement(
      TEMPLATE_CARD_NEW_EDITOR_NAME
    ) as LovelaceCardEditor;
  }

  public static getStubConfig(): TemplateNewCardConfig {
    return {
      type: `custom:${TEMPLATE_CARD_NEW_NAME}`,
    };
  }
  ...
  private _featurePosition = memoizeOne((config: TemplateNewCardConfig) => {
    if (config.vertical) {
      return "bottom";
    }
  ...
  private _displayedFeatures = memoizeOne((config: TemplateNewCardConfig) => {
