...
import { AttributeCardConfig } from "./attribute-card-config";

registerCustomCard({
    type: ENTITY_CARD_NAME,
    name: "Mushroom attribute Card",
    description: "Card for all entities and added attribute field",
});

@customElement(ENTITY_CARD_NAME)
export class AttributeCard extends LitElement implements LovelaceCard {
    public static async getConfigElement(): Promise<LovelaceCardEditor> {
        await import("./attribute-card-editor");
        return document.createElement(ENTITY_CARD_EDITOR_NAME) as LovelaceCardEditor;
    }

    public static async getStubConfig(hass: HomeAssistant): Promise<AttributeCardConfig> {
    ...
    @state() private _config?: AttributeCardConfig;

    getCardSize(): number | Promise<number> {
        return 1;
    }

    setConfig(config: AttributeCardConfig): void {
        this._config = {
            tap_action: {
                action: "more-info",
            },
