...
export function hasGroupInconsistentState(entity: HassEntity, hass: HomeAssistant) {
    const isGroup = Boolean(entity.entity_id.startsWith("group."));
    if (!isGroup) return false;

    const allEntityIds: string[] = hass.states[entity.entity_id].attributes.entity_id;
    const firstEntity: string = allEntityIds[0];

    return allEntityIds.some(
        (entityId) => hass.states[entityId].state !== hass.states[firstEntity].state
    );
}

export function getGroupMainEntity(entity: HassEntity, hass: HomeAssistant): HassEntity {
    const isGroup = Boolean(entity.entity_id.startsWith("group."));
